...
package model.feedforward;

import model.activation.ActivationFunction;
import model.activation.ActivationSigmoid;
import model.exception.NeuralNetworkError;
import model.matrix.Matrix;
import model.matrix.MatrixMath;
...
public class FeedforwardLayer implements Serializable {
	...
	/**
	 * The weight and threshold model.matrix.
	 */
	...
	/**
	 * Which model.activation function to use for this layer.
	 */
	...
	/**
	 * Clone the structure of this layer, but do not copy any model.matrix data.
	 * 
	...
	/**
	 * Take a simple double array and turn it into a model.matrix that can be used to
	 * calculate the results of the input array. Also takes into account the
	 * threshold.
	 * 
	 * @param pattern
	 * @return A model.matrix that represents the input pattern.
	 */
	...
	/**
	 * Get the weight and threshold model.matrix.
	 * @return The weight and threshold model.matrix.
	 */
	...
	/**
	 * Get the size of the model.matrix, or zero if one is not defined.
	...
	/**
	 * Determine if this layer has a model.matrix.
	 * @return True if this layer has a model.matrix.
	 */
	...
	/**
	 * Reset the weight model.matrix and threshold values to random numbers between -1
	 * and 1.
	 */
	public void reset() {
	...
	/**
	 * Assign a new weight and threshold model.matrix to this layer.
	 * @param matrix The new model.matrix.
	 */
	public void setMatrix(final Matrix matrix) {
		if (matrix.getRows() < 2) {
			throw new NeuralNetworkError(
					"Weight model.matrix includes threshold values, and must have at least 2 rows.");
		}
		if (matrix != null) {
