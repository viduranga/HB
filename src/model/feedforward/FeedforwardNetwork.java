...
import model.util.ErrorCalculation;
...
public class FeedforwardNetwork implements Serializable {
	 ...
	 * @throws MatrixException A model.matrix error occurred.
	 ...
	 * same structure, and have the same model.matrix values.
	 * 
	 * @param other
	...
	public boolean equals(final FeedforwardNetwork other) {
		...
		for (final FeedforwardLayer layer : this.getLayers()) {
			...
			// if they both have a model.matrix, then compare the matrices
			if ((layer.getMatrix() != null) && (otherLayer.getMatrix() != null)) {
	...
	/**
	 * Get the size of the weight and threshold model.matrix.
	...
	/**
	 * Reset the weight model.matrix and the thresholds.
	 * 
